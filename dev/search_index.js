...

{
    ...
    "category": "section",
    "text": "The first approach is to use Anaconda or the system python distribution and ensure that you can import the following packages. By far, the easiest solution is to use Anaconda, which can easily install the Basemap dependency.1.1 Python dependenciesmatplotlib\nbasemap\nscipy\ncmocean1.2 Testing Python installationNo matter the approach used, it is recommended to test the python environment and see if you can import the required python packages.#python\n>>> import mpl_toolkits.basemap as basemap\n>>> import matplotlib.pyplot as plt\n>>> import cmocean as cm\n>>> import scipy as scNote. Installing Basemap with pip for python 3.6+ seems problematic. That is why it is strongly suggested to use Anaconda distribution. See approach no 2a below for an installation with Anaconda1.2 Building PyCall After the confirmation that the Python dependencies can be loaded in Python, the user needs to build PyCall with the same Python version. Alternatively, if PyCall is already built, it may be only a matter of installing the Python dependencies with the PyCall\'s Python version by using pip.ENV[\"PYTHON\"]=\"path_to_python_distribution\"\npkg> build PyCall"
},

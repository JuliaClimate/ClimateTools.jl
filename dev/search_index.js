...
{
    "location": "installation/#Approach-no.-1-Use-Anaconda-or-main-system-python-distribution-1",
    "page": "Installation",
    "title": "Approach no. 1 Use Anaconda or main system python distribution",
    "category": "section",
    "text": "The first approach is to use Anaconda or the system python distribution and ensure that you can import the following packages. By far, the easiest solution is to use Anaconda, which can easily install the Basemap dependency.1.1 Python dependenciesmatplotlib\nbasemap\ncmocean1.2 Testing Python installationNo matter the approach used, it is recommended to test the python environment and see if you can import the required python packages.#python\n>>> import mpl_toolkits.basemap as basemap\n>>> import matplotlib.pyplot as plt\n>>> import cmocean as cmNote. Installing Basemap with pip for python 3.6+ seems problematic. That is why it is strongly suggested to use Anaconda distribution. See approach no 2a below for an installation with Anaconda1.2 Building PyCall After the confirmation that the Python dependencies can be loaded in Python, the user needs to build PyCall with the same Python version. Alternatively, if PyCall is already built, it may be only a matter of installing the Python dependencies with the PyCall\'s Python version by using pip.ENV[\"PYTHON\"]=\"path_to_python_distribution\"\npkg> build PyCall"
},
...

{
    ...
    "category": "section",
    "text": "once the python dependencies are properly installed you can then install ClimateTools in Julia.pkg> add ClimateTools # Tagged release"
},
...
{
    "location": "datasets/#Discontinuous-temporal-(e.g.-resampling)-1",
    "page": "Datasets",
...
{
    "location": "functions/#ClimateTools.customthresunder-Tuple{ClimGrid,Any}",
    "page": "Index",
    "title": "ClimateTools.customthresunder",
    "category": "method",
    "text": "customthresunder(C::ClimGrid, thres)\n\ncustomthresunder, annual number of days under a specified threshold.\n\nLet TS[i,j] be a daily time serie value on day i in year j. Count the number of days where:\n\nTS[i,j] < thres.\n\n\n\n\n\n"

...
const _HAS_GEOMAKIE_TEST_DEPS = Base.find_package("GeoMakie") !== nothing && Base.find_package("CairoMakie") !== nothing

if _HAS_GEOMAKIE_TEST_DEPS
    @eval using GeoMakie
    @eval using CairoMakie
end

_geomakie_test_ready() = _HAS_GEOMAKIE_TEST_DEPS && ClimateTools._ensure_plotting_extension_loaded()

@testset "GeoMakie plotting" begin
    if !_geomakie_test_ready()
        @test_skip false
    else
